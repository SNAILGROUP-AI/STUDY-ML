...
- **로지스틱 회귀식의 가중치의 이해**
    ...
    - X가 참일 때의 회귀식과 거짓일 때의 회귀식은 다음과 같음
...
- **결론**
    - 승산비의 신뢰구간에 1이 존재하는 경우 해당 설명변수의 변동이 반응변수에 미치는 영향력이 무의미하다고 판단함
    - 즉, 로지스틱 회귀식에서 임의의 설명변수 x의 가중치 $w$ 를 지수로 가지는 지수함수 $e^w$ 의 값에 대하여
    - 그 신뢰구간에 1이 존재하는 경우 해당 설명변수가 무의미하다고 판단함
